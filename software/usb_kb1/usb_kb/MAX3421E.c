...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	alt_u8 r;
	int return_code;
	return_code = alt_avalon_spi_command(0x00000140, 0, 1, &(reg+2), 0, &r, ALT_AVALON_SPI_COMMAND_MERGE);
	return_code = alt_avalon_spi_command(0x00000140, 0, 1, val, 0, &r, 0);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (alt_u32 i = 0; i < nbytes; i++) {
		if (i < nbytes - 1) {
			return_code = alt_avalon_spi_command(0x00000140, 0, 1, &(reg+2), 0, &r, ALT_AVALON_SPI_COMMAND_MERGE);
			return_code = alt_avalon_spi_command(0x00000140, 0, 1, data[i], 0, &r, ALT_AVALON_SPI_COMMAND_MERGE);
		} else {
			return_code = alt_avalon_spi_command(0x00000140, 0, 1, &(reg+2), 0, &r, ALT_AVALON_SPI_COMMAND_MERGE);
			return_code = alt_avalon_spi_command(0x00000140, 0, 1, data[i], 0, &r, 0);
		}
		data++;
		if (return_code < 0) printf("error");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int return_code
	alt_u8 read;
	return_code = alt_avalon_spi_command(0x00000140, 0, 1, &reg, 0, &read, ALT_AVALON_SPI_COMMAND_MERGE);		// write and read separately?
	return_code = alt_avalon_spi_command(0x00000140, 0, 0, &reg, 1, &read, 0);
	if (return_code < 0) printf("error");
	return read;
}
//multiple-byte write
...
//	alt_u8 read;
	...
	for (alt_u32 i = 0; i < nbytes; i++) {
		if (i < nbytes - 1) {
			return_code = alt_avalon_spi_command(0x00000140, 0, 1, &reg, 0, data[i], ALT_AVALON_SPI_COMMAND_MERGE);
			return_code = alt_avalon_spi_command(0x00000140, 0, 0, &reg, 1, data[i], ALT_AVALON_SPI_COMMAND_MERGE);
		} else {
			return_code = alt_avalon_spi_command(0x00000140, 0, 1, &reg, 0, data[i], ALT_AVALON_SPI_COMMAND_MERGE);
			return_code = alt_avalon_spi_command(0x00000140, 0, 0, &reg, 1, data[i], 0);
		}
		if (return_code < 0) printf("error");
